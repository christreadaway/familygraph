...
//   [magic:8 'FGRAPH01'][iv:12][tag:16][ciphertext: gzip(<sqlite-bytes>)]
...
const MAGIC = Buffer.from('FGRAPH01', 'utf8');
const SALT_LEN = 16;
const IV_LEN = 12;
const TAG_LEN = 16;
...
function decryptedRestore(backupPath, dbOutPath, passphrase) {
  ...
  if (!blob.slice(0, MAGIC.length).equals(MAGIC)) throw new Error('not a Family Graph backup file');
...
function plainCopyBackup(dbPath, backupsDir) {
  ...
  const out = path.join(backupsDir, `family-graph-${stamp}.sqlite`);
...
async function hotBackup(db, backupsDir, { passphrase = null } = {}) {
  fs.mkdirSync(backupsDir, { recursive: true, mode: 0o700 });
  const stamp = new Date().toISOString().replace(/[:]/g, '-');
  const tmp = path.join(backupsDir, `.tmp-family-graph-${stamp}.sqlite`);
  await db.backup(tmp);
  if (passphrase) {
    const out = path.join(backupsDir, `family-graph-${stamp}.family-graph-backup`);
