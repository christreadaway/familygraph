...
// the email tells the colleague "you have N conflicts waiting in Sanctus,
...
function assignTemplate({ count, expiresAt, dashboardUrl, assignee, ttlHours, institution }) {
  ...
  const subject = `[Sanctus] ${count} family-data conflict${count === 1 ? '' : 's'} assigned to you (resolve within ${ttlHours}h)`;
  const text = [
    `Hi,`,
    ``,
    `You've been assigned ${count} ${inst}family-data conflict${count === 1 ? '' : 's'} in Sanctus for review.`,
  ...
  const html = `<!doctype html><html><body style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#222;line-height:1.5">
    <p>Hi,</p>
    <p>You've been assigned <strong>${count} ${inst}family-data conflict${count === 1 ? '' : 's'}</strong> in Sanctus for review.<br>
    ...
    <p style="color:#888;font-size:12px;margin-top:32px">— Sanctus</p>
  </body></html>`;
...
function reminderTemplate({ count, expiresAt, dashboardUrl, assignee, institution }) {
  ...
  const link = `${dashboardUrl.replace(/\/+$/, '')}/conflicts?assigned_to=${encodeURIComponent(assignee)}`;
  const remaining = _humanRemaining(expiresAt);
  const subject = `[Sanctus] Reminder: ${count} ${inst}conflict${count === 1 ? '' : 's'} assigned to you, ${remaining} left`;
  const text = [
    `Hi,`,
    ``,
    `Just a reminder: you have ${count} ${inst}family-data conflict${count === 1 ? '' : 's'} still open in Sanctus.`,
    ``,
    `Time remaining: ${remaining}`,
    `Hard deadline: ${_formatDeadline(expiresAt)}`,
    ``,
  ...
  const html = `<!doctype html><html><body style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#222;line-height:1.5">
    <p>Hi,</p>
    <p>Just a reminder: you have <strong>${count} ${inst}family-data conflict${count === 1 ? '' : 's'}</strong> still open in Sanctus.</p>
...
function expiredTemplate({ count, dashboardUrl, assignee, institution }) {
  ...
  const subject = `[Sanctus] Your assignment of ${count} conflict${count === 1 ? '' : 's'} has expired`;
  const text = [
    `Hi,`,
    ``,
    `Your assignment of ${count} ${inst}family-data conflict${count === 1 ? '' : 's'} in Sanctus has expired and the conflict${count === 1 ? ' is' : 's are'} back in the unassigned pool.`,
    ``,
  ...
  const html = `<!doctype html><html><body style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#222;line-height:1.5">
    <p>Hi,</p>
    <p>Your assignment of <strong>${count} ${inst}family-data conflict${count === 1 ? '' : 's'}</strong> in Sanctus has expired and the conflict${count === 1 ? ' is' : 's are'} back in the unassigned pool.</p>
