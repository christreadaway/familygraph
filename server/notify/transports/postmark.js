...
// Server token comes from the FAMILY_GRAPH_POSTMARK_TOKEN env var (never stored
...
function send({ token, from, messageStream }, msg) {
  if (!token) return Promise.reject(new Error('postmark: missing FAMILY_GRAPH_POSTMARK_TOKEN'));
