...
test('templates > assignTemplate produces subject + text + html with TTL and link', () => {
  ...
  assert.match(t.text, /https:\/\/family-graph\.example\.org\/conflicts\?assigned_to=sarah%40x\.org/);
  assert.match(t.text, /Time remaining/);
  assert.match(t.html, /<a href="https:\/\/family-graph\.example\.org\/conflicts\?assigned_to=sarah%40x\.org"/);
});
